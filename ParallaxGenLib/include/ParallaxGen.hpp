...
public:
  ...
  patchMeshes(const std::vector<std::function<std::unique_ptr<PatcherShader>(std::filesystem::path, nifly::NifFile *)>>
                  &Patchers,
              const std::unordered_map<std::wstring, int> *ModPriority, const bool &MultiThread = true,
...
private:
  ...
  auto processNIF(
      const std::vector<std::function<std::unique_ptr<PatcherShader>(std::filesystem::path, nifly::NifFile *)>>
          &Patchers,
      const std::unordered_map<std::wstring, int> *ModPriority, const std::filesystem::path &NIFFile,
      nlohmann::json *DiffJSON, const bool &PatchPlugin = true, const bool &Dry = false,
      std::unordered_map<std::wstring, std::tuple<std::set<NIFUtil::ShapeShader>, std::unordered_set<std::wstring>>>
  ...
  auto processShape(
      ...
      const std::unordered_map<std::wstring, int> *ModPriority, bool &ShapeModified, bool &ShapeDeleted,
