...
private:
  ...
  // If nif or texture paths have these components dynamic cubemaps will be
  // ignored for CM patching
  // TODO make this configurable
  static inline const std::vector<std::wstring> dynCubemap_ignore_list = {
      L"armor", L"weapons"};
  ...
  // store the suffixes for each texture slot. First element of each vector is
...
public:
  static inline const std::string parallax_state_file =
      "PARALLAXGEN_DONTDELETE";
  ...
  ParallaxGen(const std::filesystem::path output_dir, ParallaxGenDirectory *pgd,
              ParallaxGenD3D *pgd3d, const bool optimize_meshes = false,
...
private:
  ...
  ParallaxGenTask::PGResult convertHeightMapToComplexMaterial(
      const std::filesystem::path &height_map) const;
  // processes a NIF file (enable parallax if needed)
  ParallaxGenTask::PGResult
  processNIF(const std::filesystem::path &nif_file,
             const std::vector<nlohmann::json> &tpbr_configs) const;
  // processes a shape within a NIF file
  ParallaxGenTask::PGResult
  processShape(const std::filesystem::path &nif_file,
               const std::vector<nlohmann::json> &tpbr_configs,
               nifly::NifFile &nif, const uint32_t shape_block_id,
               nifly::NiShape *shape, bool &nif_modified,
  ...
  ParallaxGenTask::PGResult shouldApplyTruePBRConfig(
      ...
      const std::vector<nlohmann::json> &tpbr_configs, nifly::NifFile &nif,
      const uint32_t shape_block_id, nifly::NiShape *shape,
      nifly::NiShader *shader,
      const std::array<std::string, 9> &search_prefixes, bool &enable_result,
      std::vector<std::tuple<nlohmann::json, std::string>> &truepbr_data) const;
  // check if complex material should be enabled on shape
  static inline const int cm_slot_search[2] = {
      0, 1}; // Checks match for diffuse first, then normal
  ParallaxGenTask::PGResult shouldEnableComplexMaterial(
      const std::filesystem::path &nif_file, nifly::NifFile &nif,
      const uint32_t shape_block_id, nifly::NiShape *shape,
      nifly::NiShader *shader,
      const std::array<std::string, 9> &search_prefixes, bool &enable_result,
      std::string &matched_path) const;
  // check if vanilla parallax should be enabled on shape
  static inline const int parallax_slot_search[2] = {
      0, 1}; // Checks match for diffuse first, then normal
  ParallaxGenTask::PGResult
  shouldEnableParallax(const std::filesystem::path &nif_file,
                       nifly::NifFile &nif, const uint32_t shape_block_id,
                       nifly::NiShape *shape, nifly::NiShader *shader,
                       const std::array<std::string, 9> &search_prefixes,
                       const bool has_attached_havok, bool &enable_result,
                       std::string &matched_path) const;
  ...
  ParallaxGenTask::PGResult applyTruePBRConfigOnShape(
      nifly::NifFile &nif, nifly::NiShape *shape, nifly::NiShader *shader,
  ...
  ParallaxGenTask::PGResult
  enableTruePBROnShape(nifly::NifFile &nif, nifly::NiShape *shape,
                       nifly::NiShader *shader, nlohmann::json &truepbr_data,
                       const std::string &matched_path,
                       bool &nif_modified) const;
  // enables complex material on a shape in a NIF
  ParallaxGenTask::PGResult
  enableComplexMaterialOnShape(nifly::NifFile &nif, nifly::NiShape *shape,
                               nifly::NiShader *shader,
                               const std::string &matched_path,
  ...
  ParallaxGenTask::PGResult enableParallaxOnShape(
      nifly::NifFile &nif, nifly::NiShape *shape, nifly::NiShader *shader,
      const std::string &matched_path, bool &nif_modified) const;

  // Gets all the texture prefixes for a textureset. ie. _n.dds is removed etc.
  // for each slot
  static const std::array<std::string, 9>
  getSearchPrefixes(nifly::NifFile &nif, nifly::NiShape *shape);
  ...
  static nifly::Vector2 auto_uv_scale(const std::vector<nifly::Vector2> *uvs,
  ...
  static std::array<std::string, 9>
  getTexturePaths(const std::array<std::string, 9> &matched_paths);
};
