...
private:
  ...
  // downstream users
  std::vector<std::wstring> extension_blocklist = {L".bsa", L".esp", L".esl",
                                                   L".esm"};
...
public:
  ...
  findFiles(const bool lower = false,
            const std::vector<std::wstring> &glob_list_allow =
                std::vector<std::wstring>(),
            const std::vector<std::wstring> &glob_list_deny =
                std::vector<std::wstring>(),
            const std::vector<std::wstring> &archive_list_deny =
  ...
  std::vector<std::wstring>
  getPluginLoadOrder(const bool trim_extension = false) const;

  // Helpers
  static std::filesystem::path getPathLower(const std::filesystem::path path);
  static bool pathEqualityIgnoreCase(const std::filesystem::path path1,
...
private:
  ...
  std::vector<std::wstring>
  findBSAFilesFromPluginName(const std::vector<std::wstring> &bsa_file_list,
                             const std::wstring &plugin_prefix) const;

  // helpers
  BethesdaFile getFileFromMap(const std::filesystem::path &file_path) const;
  void updateFileMap(const std::filesystem::path &file_path,
                     std::shared_ptr<BSAFile> bsa_file);
  static std::vector<LPCWSTR>
  convertWStringToLPCWSTRVector(const std::vector<std::wstring> &original);
  static bool checkGlob(const LPCWSTR &str, LPCWSTR &winning_glob,
                        const std::vector<LPCWSTR> &glob_list);
};
