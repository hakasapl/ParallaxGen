...
BethesdaDirectory::getFileMap() const {
  return fileMap;
}
...
vector<std::byte> BethesdaDirectory::getFile(const fs::path rel_path) const {
  ...
  if (bsa_struct == nullptr) {
    ...
    if (logging) {
      spdlog::trace(L"Reading BSA file from {}: {}", bsa_path.wstring(),
                    rel_path.wstring());
    }
    ...
    if (file) {
      ...
      try {
        ...
        if (logging) {
          spdlog::error(L"Failed to read file {}: {}", rel_path.wstring(),
...
fs::path BethesdaDirectory::getDataPath() const { return data_dir; }

vector<fs::path>
BethesdaDirectory::findFiles(const bool lower,
                             const vector<wstring> &glob_list_allow,
                             const vector<wstring> &glob_list_deny,
                             const vector<wstring> &archive_list_deny) const {
  // find all keys in fileMap that match pattern
  vector<fs::path> found_files;

  // Create LPCWSTR vectors from wstrings
  vector<LPCWSTR> glob_list_allow_cstr =
      convertWStringToLPCWSTRVector(glob_list_allow);
  vector<LPCWSTR> glob_list_deny_cstr =
      convertWStringToLPCWSTRVector(glob_list_deny);
  vector<LPCWSTR> archive_list_deny_cstr =
      convertWStringToLPCWSTRVector(archive_list_deny);
  ...
  for (const auto &[key, value] : fileMap) {
    ...
    // Check allowlist
    if (!glob_list_allow_cstr.empty() &&
        !checkGlob(key_str, last_winning_glob_allow, glob_list_allow_cstr)) {
      continue;
    }

    // Check denylist
    if (!glob_list_deny_cstr.empty() &&
        checkGlob(key_str, last_winning_glob_deny, glob_list_deny_cstr)) {
      continue;
    }
    ...
    if (value.bsa_file != nullptr) {
      ...
      if (checkGlob(bsa_file, last_winning_glob_archive_deny,
                    archive_list_deny_cstr)) {
        continue;
      }
    }
...
void BethesdaDirectory::addLooseFilesToMap() {
  ...
  for (const auto &entry : fs::recursive_directory_iterator(
           data_dir, fs::directory_options::skip_permission_denied)) {
  ...
  for (auto bsa_iter = bsa_obj.begin(); bsa_iter != bsa_obj.end(); ++bsa_iter) {
    ...
    try {
      ...
      for (const auto &entry : file_name) {
        ...
        if (logging) {
          spdlog::trace(L"Adding file from BSA {} to file map: {}", bsa_name,
                        cur_path.wstring());
        }
      ...
      if (logging) {
        spdlog::warn(L"Failed to get file pointer from BSA, skipping {}: {}",
                     bsa_name, convertToWstring(e.what()));
      }
      continue;
    }
  }
...
vector<wstring> BethesdaDirectory::getBSAPriorityList() const {
  ...
  for (wstring plugin : load_order) {
    ...
    vector<wstring> cur_found_bsas =
        findBSAFilesFromPluginName(all_bsa_files, plugin);
  ...
  if (logging) {
    ...
    for (wstring bsa : all_bsa_files) {
      if (find(out_bsa_order.begin(), out_bsa_order.end(), bsa) ==
          out_bsa_order.end()) {
...
vector<wstring>
BethesdaDirectory::getPluginLoadOrder(const bool trim_extension) const {
  if (logging) {
  ...
  /*if (!fs::exists(lo_file)) {
          if (logging) {
                  spdlog::critical("loadorder.txt not found. If you are using
  mod organizer be sure to launch this program through MO2.");
                  exitWithUserInput(1);
          }
          else {
...
bool BethesdaDirectory::pathEqualityIgnoreCase(const fs::path path1,
                                               const fs::path path2) {
...
vector<wstring> BethesdaDirectory::getBSAFilesFromINIs() const {
  ...
  for (string field : ini_bsa_fields) {
    ...
    for (fs::path ini_path : ini_file_order) {
      wstring cur_val =
          readINIValue(ini_path, L"Archive", convertToWstring(field), logging,
                       first_ini_read);

      if (logging) {
        spdlog::trace(L"Found ini key pair from INI {}: {}: {}",
                      ini_path.wstring(), convertToWstring(field), cur_val);
      }
    ...
    if (logging) {
      spdlog::trace(L"Found BSA files from INI field {}: {}",
                    convertToWstring(field), ini_val);
    }
...
vector<wstring> BethesdaDirectory::findBSAFilesFromPluginName(
    const vector<wstring> &bsa_file_list, const wstring &plugin_prefix) const {
  if (logging) {
...
bool BethesdaDirectory::file_allowed(const fs::path file_path) const {
  ...
  if (std::find(extension_blocklist.begin(), extension_blocklist.end(),
                file_extension) != extension_blocklist.end()) {
...
BethesdaDirectory::getFileFromMap(const fs::path &file_path) const {
...
void BethesdaDirectory::updateFileMap(
    ...
    shared_ptr<BethesdaDirectory::BSAFile> bsa_file) {
  fs::path lower_path = getPathLower(file_path);
...
bool BethesdaDirectory::checkIfAnyComponentIs(
    const fs::path path, const vector<wstring> &components) {
...
vector<LPCWSTR> BethesdaDirectory::convertWStringToLPCWSTRVector(
    const vector<wstring> &original) {
...
bool BethesdaDirectory::checkGlob(const LPCWSTR &str, LPCWSTR &winning_glob,
                                  const vector<LPCWSTR> &glob_list) {
